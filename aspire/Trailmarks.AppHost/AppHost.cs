...
// Map ASPIRE_RESOURCE_SERVICE_ENDPOINT_URL to DOTNET_RESOURCE_SERVICE_ENDPOINT_URL
builder.Configuration["DOTNET_RESOURCE_SERVICE_ENDPOINT_URL"] = 
    builder.Configuration["ASPIRE_RESOURCE_SERVICE_ENDPOINT_URL"];

// Configure OTLP endpoint for dashboard
builder.Configuration["DOTNET_DASHBOARD_OTLP_ENDPOINT_URL"] = "https://localhost:4317";
...
//var keycloak = builder.AddContainer("keycloak-server", "quay.io/keycloak/keycloak", "26.0.7")
...
//    .WithHttpEndpoint(port: 8180, targetPort: 8080, name: "keycloak-http")
...
var backend = builder.AddProject<Projects.TrailmarksApi>("backend")
    ...
    .WithHttpEndpoint(port: 8080, name: "backend-http")
    .WaitFor(postgres)
    //.WaitFor(keycloak)
    ;
